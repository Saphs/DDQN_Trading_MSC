...
class TestEnvironment(unittest.TestCase):
    ...
    def test_state_mode_1(self):
        ...
        env: Environment = Environment(data=df, state_mode=1, action_name=self._action_name)
        self.assertEqual(env.state_size, 4)
        self.assertEqual(env.starting_offset, 0)
    ...
    def test_state_mode_2(self):
        df: DataFrame = DataFrame(self._mock_data.copy())
        env: Environment = Environment(data=df, state_mode=2, action_name=self._action_name)
    ...
    def test_state_mode_3(self):
        df: DataFrame = DataFrame(self._mock_data.copy())
        env: Environment = Environment(data=df, state_mode=3, action_name=self._action_name)
    ...
    def test_state_mode_4(self):
        df: DataFrame = DataFrame(self._mock_data.copy())
        env: Environment = Environment(data=df, state_mode=4, action_name=self._action_name)
    ...
    def test_state_mode_5(self):
        df: DataFrame = DataFrame(self._mock_data.copy())
        env: Environment = Environment(data=df, state_mode=5, action_name=self._action_name, window_size=1)
        self.assertEqual(env.state_size % 4, 0)
        self.assertEqual(env.state_size, 4)
        self.assertEqual(env.starting_offset, 0)

        env: Environment = Environment(data=df, state_mode=5, action_name=self._action_name, window_size=2)
        self.assertEqual(env.state_size % 4, 0)
        self.assertEqual(env.state_size, 8)
        self.assertEqual(env.starting_offset, 1)

        env: Environment = Environment(data=df, state_mode=5, action_name=self._action_name, window_size=3)
        self.assertEqual(env.state_size % 4, 0)
        self.assertEqual(env.state_size, 12)
        self.assertEqual(env.starting_offset, 2)
